    ...
    public class ProductsController(DataContext context, IWebHostEnvironment webHostEnvironment) : Controller
    {
        ...
        public async Task<IActionResult> Index(string slug = "", int p = 1)
        {
            ...
            if (slug == "")
            {
                ...
                List<Product> products = await _context.Products.Include(x => x.Category).OrderByDescending(x => x.Id).Skip((p - 1) * pageSize).Take(pageSize).ToListAsync();
            ...
            return View(await productsByCategory.Include(x => x.Category).OrderByDescending(x => x.Id).Skip((p - 1) * pageSize).Take(pageSize).ToListAsync());
